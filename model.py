...
class NeuralCF(nn.Module):
    def __init__(self,num_users,num_items,embedding_size,num_layers,num_director,num_genre):
        ...
        self.fc1 = nn.Linear(2*embedding_size +num_genre , embedding_size)
        self.fc2 = nn.Linear(int(embedding_size*(2**(num_layers-1))) * 2 +(num_genre) , int(embedding_size*(2**(num_layers-1))))
    ...
    def forward(self,user_indices,item_indices,director,genre):
        ...
        item_gmf = torch.cat([x2, x5,genre], -1)
        item_mlp = torch.cat([x4, x6,genre], -1)
        
        
        item_feature_gmf = self.fc1(item_gmf)
        item_feature_mlp = self.fc2(item_mlp)
